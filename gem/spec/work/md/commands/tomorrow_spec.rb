...
RSpec.describe Work::Md::Commands::Tomorrow do
  ...
  context 'executing' do
    let(:expected_md_file) { "#{Work::Md::Config.work_dir}/#{tomorrow.strftime('%Y/%m/%d')}.md" }
    let(:expected_md_file_dir) { "#{Work::Md::Config.work_dir}/#{tomorrow.strftime('%Y/%m')}" }
    ...
    context 'opening the md file in the work dir' do
      it 'when editor not set' do
        allow(::TTY::Editor).to(
          receive(:open)
          .with("#{tomorrow.strftime('%Y/%m/%d')}.md")
          .and_return(true)
        )
